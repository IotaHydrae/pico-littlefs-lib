...
int main(void)
{
	...
	lfs_ssize_t nread =
		lfs_file_read(&lfs, &file, readback, sizeof(readback) - 1);
	if (nread < 0) {
	...
	err = lfs_file_open(&lfs, &file, filename, LFS_O_WRONLY | LFS_O_APPEND);
	if (err != LFS_ERR_OK) {
		printf("  Append open FAIL: %s\n", lfs_err_str(err));
	} else {
		wrote = lfs_file_write(&lfs, &file, append_msg,
				       strlen(append_msg));
	...
	if (err == LFS_ERR_OK) {
		...
		char buf[64];
		lfs_ssize_t rd;
		while ((rd = lfs_file_read(&lfs, &file, buf, sizeof(buf) - 1)) >
		       0) {
			buf[rd] = '\0';
			printf("  %s", buf);
		}
